...
/*!*/       validation.Text = productBuilder.BuildEmail(emailbox.Text).BuildName(textBox1.Text)
                                            .BuildModel(words[0], words[1]).BuildSize(clone.size, clone.weight)
                                            .BuildDate(dateTimePicker1.Value).BuildPrice()
                                            .GetResult();
        }
